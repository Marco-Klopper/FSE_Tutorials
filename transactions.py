...
def add_transaction(
    ...
    if transaction_type not in TRANSACTION_TYPES:
        raise ValueError('ValueError: transaction_type is invalid. Ensure value is either "income" or "expense".')
    
    if amount < 0:
        raise ValueError('ValueError: amount is invalid. Ensure value is positive')
    

    new_transaction = {'description': description, 'amount': Decimal(amount), 'type': transaction_type}
...
def calculate_balance(transactions: List[dict]) -> Decimal:
    ...
    for transaction in transactions:
        ...
        current_balance = current_balance + tnx_amount        
...
def get_income_total(transactions: List[dict]) -> Decimal:
    ...
    for transaction in transactions:
        if transaction['type'] == 'income':
            tnx_amount = transaction['amount']
            income_balance = income_balance + tnx_amount 

    return Decimal(income_balance)
...
def get_expense_total(transactions: List[dict]) -> Decimal:
    ...
    for transaction in transactions:
        if transaction['type'] == 'expense':
            tnx_amount = transaction['amount']
            expense_balance = expense_balance + tnx_amount 

    return Decimal(expense_balance)
...
def check_budget(balance: Decimal, budget_limit: Decimal) -> Tuple[bool, str]:
    ...
    if balance > budget_limit:
        return (False, f'Budget exceeded! Overspent by {format_currency(balance - budget_limit)}.')
...
def display_transactions(transactions: List[dict]) -> None:
    ...
    Args:
        transactions: A list of transaction dictionaries.
    """
    print("|     description     |       amount      |       type        |")
    for tnx in transactions:
        print(f"|     {tnx['description']}     |       {format_currency(tnx['amount'])}      |       {tnx['type']}        |")
